        ...
        <div className="grid grid-cols-1 md:grid-cols-2">
          ...
          <div className="p-6 sm:p-10">
            <div className="flex items-center gap-2 text-xs text-[#9CA3AF] mb-6">
              {[1,2,3].map((s) => (
                <div key={s} className={`h-1.5 w-10 rounded-full ${step >= s ? 'bg-[#2563EB]' : 'bg-[#1D2430]'}`} />
              ))}
            ...
            {step === 1 && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {CATEGORIES.map((c) => {
                    ...
                    return (
                      <button
                        key={c.key}
                        onClick={() => setInterests((prev) => active ? prev.filter((k) => k !== c.key) : [...prev, c.key])}
                        className={`px-3 py-2 rounded-xl border text-sm inline-flex items-center gap-2 transition
                          ${active ? 'bg-[#2563EB] border-[#2563EB] text-white' : 'border-[#1D2430] hover:bg-[#11151d]'}`}
                      >
                ...
                <div className="pt-6 flex gap-3">
                  <button onClick={() => setStep(2)} className="h-11 w-full rounded-2xl bg-[#2563EB] hover:bg-[#1D4ED8] transition">
                    Continuar
                  </button>
                </div>
              </motion.div>
                ...
                <div className="rounded-2xl border border-[#1D2430] p-4 flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    ...
                    <div className="text-[#9CA3AF] text-xs">
                      Usamos sua posição aproximada para priorizar itens próximos. {method === 'gps' ? '✓ Capturado via GPS' : '✓ Estimado via IP'}
                    </div>
                  </div>
                </div>

                <div className="flex gap-3 pt-2">
                  <button onClick={() => setStep(1)} className="h-11 w-1/2 rounded-2xl border border-[#1D2430] hover:bg-[#11151d] transition">Voltar</button>
                  <button onClick={() => setStep(3)} className="h-11 w-1/2 rounded-2xl bg-[#2563EB] hover:bg-[#1D4ED8] transition">Continuar</button>
                </div>
              </motion.div>
            )}
              ...
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {CONDITIONS.map((c) => {
                    ...
                    const label = c === 'semi' ? 'seminovo' : (c === 'pecas' ? 'peças' : 'novo');
                    return (
                      <button
                        key={c}
                        onClick={() => setConditions((prev) => active ? prev.filter((k) => k !== c) : [...prev, c])}
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={() => setStep(2)} className="h-11 w-1/2 rounded-2xl border border-[#1D2430] hover:bg-[#11151d] transition">Voltar</button>
                  <button onClick={save} className="h-11 w-1/2 rounded-2xl bg-[#2563EB] hover:bg-[#1D4ED8] transition">Concluir</button>
                </div>
              </motion.div>
            )}
          </div>
